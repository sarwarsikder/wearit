...
export class AuthService {
  ...
  connectFacebook(accessToken: string): Promise<any> {
    return this.restangular.all('connect/facebook').customPOST({ accessToken }).toPromise();
  }

  connectGoogle(accessToken: string): Promise<any> {
    return this.restangular.all('connect/google').customPOST({ accessToken }).toPromise();
  }
