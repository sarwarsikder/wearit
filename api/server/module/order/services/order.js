...
exports.create = async (data, user) => {
  try {
    ...
    for (const product of mappingProducts) {
      ...
      if (
        !freeShip &&
        !product.product.freeShip &&
        product.shop.storeWideShipping
      ) {
        shippingPrice = product.shop.shippingSettings.defaultPrice;
        if (product.quantity > 1) {
          shippingPrice +=
            product.shop.shippingSettings.perQuantityPrice *
            (product.quantity - 1);
        }
      ...
      orderDetail.totalPrice =
        Math.round(
          (productPrice + orderDetail.taxPrice + shippingPrice) * 100
        ) / 100; // TODO - round me
...
exports.updateProductQuantity = async (orderDetail) => {
  try {
    const orderDetails = Array.isArray(orderDetail)
      ...
      : [orderDetail];
    return Promise.all(
      orderDetails.map(({ productId, productVariantId, quantity }) =>
...
orderQ.process(async (job, done) => {
  try {
    ...
    const orderDetails = await DB.OrderDetail.find({ orderId }).populate(
      "shop"
    );
    let customer;
    if (order.customerId) {
